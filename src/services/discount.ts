...
class DiscountService extends MedusaDiscountService {
    ...
    async calculateDiscountForLineItem(
        ...
        return await this.atomicPhase_(async (transactionManager) => {
            ...
            const includesTax =
                this.featureFlagRouter_.isFeatureEnabled(TaxInclusivePricingFeatureFlag.key) && lineItem.includes_tax;
            ...
            if (type === DiscountRuleType.PERCENTAGE) {
            ...
            } else if (type === DiscountRuleType.FIXED && allocation === DiscountAllocation.TOTAL) {
                ...
                let discountedItems = calculationContextData.items.filter((item) => item.allow_discounts);

                if (discount.rule.conditions.length > 0) {
                    for (const condition of discount.rule.conditions) {
                        const operatorInclude = condition.operator === DiscountConditionOperator.IN;
                        ...
                        if (condition.type === DiscountConditionType.PRODUCT_COLLECTIONS) {
                            const collectionIds = condition.product_collections.map((c) => c.id);
                            discountedItems = discountedItems.filter((item) => {
                                const include = collectionIds.includes(item.variant.product.collection_id);
                                return operatorInclude ? include : !include;
                            });
                        }
                ...
                const totals = await this.newTotalsService_.getLineItemTotals(discountedItems, {
