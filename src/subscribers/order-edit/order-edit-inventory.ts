...
const addRemoveItem = (
  lineItem: LineItem,
  changeType:
    | OrderEditItemChangeType.ITEM_ADD
    | OrderEditItemChangeType.ITEM_REMOVE
): VariantInventory => {
...
const updateItem = (
  lineItemBefore: LineItem,
  lineItemAfter: LineItem
): VariantInventory => {
...
export default async function orderEditHandler({
  data,
  container,
}: SubscriberArgs<{ id: string }>) {
  const orderEditService =
    container.resolve<OrderEditService>("orderEditService");
